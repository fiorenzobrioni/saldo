...
interface TransactionDao {
    ...
    /**
     * Candidate groups for the recurrence scan, keyed by exact amount (Fase 19,
     * ADR 43): confirmed movements not generated by a rule, grouped by type,
     * account, category, currency and amount. Selection happens here in SQL so
     * the scan cost stays flat as the ledger grows: the detector only ever sees
     * these few groups. Rows excluded from statistics stay out: a loan to a
     * person or a deliberately excluded movement is not representable by a
     * rule, which always generates plain statistical movements.
     *
     * Runs ONLY on the explicit user action in the hub, never on open (ADR 43).
     */
    @Query(
        """
    ...
    /**
     * Candidate groups for the recurrence scan, keyed by case-folded
     * description (Fase 19, ADR 43): the variable-bill path, where the amount
     * changes but the description repeats. `LOWER` folds ASCII only, like the
     * `LIKE` prefilter of [descriptionUsage]: the accent-insensitive merge
     * happens in Kotlin on these few groups. Same filters as
     * [recurrenceAmountGroups].
     */
    @Query(
        """
        SELECT type AS type, accountId AS accountId, categoryId AS categoryId,
