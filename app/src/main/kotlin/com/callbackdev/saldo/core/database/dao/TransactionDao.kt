...
interface TransactionDao {
     ...
     * single-currency twins with the currency test inverted, grouped by
     * (currency, local day of the movement, ADR 7). The day granularity is
     * what lets the domain convert each bucket at the rate of the movement's
     * own date. The twins stay untouched: the single-currency case never even
     * runs these.
     */

    /**
     * Foreign twin of [observeDashboardTotals]: the same seven window sums,
    ...
    /**
     * Foreign twin of [observeMonthlyTotals], per (currency, local day); the
    ...
    /**
     * [observeOtherCurrencyCount] broken down per currency, so the screen can
    ...
    /** Per-category twin of [observeSpendByCurrencyDay]; same filters as [observeCategorySpendTotals]. */
    @Query(
        """
