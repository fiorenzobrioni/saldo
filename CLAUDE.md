...
- **Versioning delle build di test**: al termine di ogni implementazione che modifica l'app (non per modifiche solo alla documentazione), incrementa `versionCode` di 1 in `app/build.gradle.kts` e allinea `versionName` allo schema `2.2.<incremento>` (lo schema segue sempre l'ultima versione pubblicata: era `0.<ultima fase completata>.<incremento>` fino alla 1.0.0, `1.0.<incremento>` fra la 1.0.0 e la 2.0.0, `2.0.<incremento>` fra la 2.0.0 e la 2.1.0, `2.1.<incremento>` fra la 2.1.0 e la 2.2.0; il prossimo traguardo pianificato è la 3.0.0, ma una minor pubblicata nel frattempo sposta lo schema, come hanno fatto la 2.1.0 e la 2.2.0). Serve ad aggiornare in place l'APK debug sul device di test senza reinstallare (e senza perdere i dati inseriti). Il bump fa parte del commit dell'implementazione, non richiede una build dedicata.
- **Keystore di debug condiviso**: `keystore/debug.keystore` (committato di proposito, `signingConfigs.debug` in `app/build.gradle.kts`) firma allo stesso modo ogni build, locale o CI. Senza un keystore condiviso il bump di versione da solo non basta: ogni macchina firmerebbe con il proprio `~/.android/debug.keystore` e Android rifiuterebbe comunque l'update in-place per firma diversa. Non toccare né rigenerare questo file: è una chiave di solo debug (nessun valore ai fini della pubblicazione), non ha bisogno di segreti/CI secrets. Le build debug hanno `applicationIdSuffix ".debug"` e label launcher `Saldo (dev)` (overlay di risorse debug): si installano **fianco a fianco** con l'app firmata di release — decisione di serie (ago 2026, ADR 48). La build *release* firmata col flag debug mantiene l'id pulito: serve agli smoke test pre-release, non al test quotidiano.
- **Release**: si pubblica su GitHub, non sul Play Store. Da ago 2026 (ADR 47, che supera la scelta dell'artefatto dell'ADR 38) la pubblica il workflow `.github/workflows/release.yml` al push di un tag `vX.Y.Z`: test+lint, APK **release minificato firmato con la chiave vera** (keystore fuori dal repo, secrets `KEYSTORE_BASE64`/`KEYSTORE_PASSWORD`/`KEY_ALIAS`/`KEY_PASSWORD` passati come `ORG_GRADLE_PROJECT_SALDO_*`), mapping R8 allegato. Il flusso umano resta: scrivere le note in `docs/release-notes/vX.Y.Z.md` (il workflow **fallisce senza il file**), bump di versione nel commit della release, merge, tag. In locale la signingConfig `release` legge le proprietà `SALDO_*` da `~/.gradle/gradle.properties`; senza proprietà la release è non firmata, con `-PsignReleaseWithDebugKey` la firma la chiave di debug (è la build per-push della CI, installabile per smoke test). **Nota migrazione**: la prima release firmata con la chiave vera non aggiorna in place le installazioni debug-signed (v2.1.0 e precedenti) — serve disinstallare, quindi backup dei dati prima.
- **Il corpo della release non è il file delle note**: nel file dentro il repo i link sono relativi (`../../README.md`) e lì sono corretti; nel corpo di una release GitHub li risolve rispetto a `/<owner>/<repo>/releases/tag/<tag>`, quindi finiscono su percorsi inesistenti e danno 404. La riscrittura in link assoluti puntati al **tag della release** (la sorgente di quella versione, non `main`, che diverge dal testo delle note) è fatta dal workflow di release, con un guardrail che fallisce se sopravvive un link relativo: la v1.0.0, la v2.0.0 e la v2.1.0 erano state pubblicate a mano tutte e tre con i link rotti, e corrette dopo. Se si aggiunge alle note una forma di riferimento nuova (immagini, `](./`), va coperta nel passo `Absolutize release-notes links` del workflow.
