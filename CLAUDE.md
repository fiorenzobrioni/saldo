...
- **Versioning delle build di test**: al termine di ogni implementazione che modifica l'app (non per modifiche solo alla documentazione), incrementa `versionCode` di 1 in `app/build.gradle.kts` e allinea `versionName` allo schema `1.0.<incremento>` (fino alla release 1.0.0 lo schema era `0.<ultima fase completata>.<incremento>`; la prossima versione pubblicata sarà la 2.0.0). Serve ad aggiornare in place l'APK debug sul device di test senza reinstallare (e senza perdere i dati inseriti). Il bump fa parte del commit dell'implementazione, non richiede una build dedicata.
...
- **Release**: si pubblica su GitHub, non sul Play Store (ADR 38 in PLANNING.md). Il tag `vX.Y.Z` sul commit da rilasciare è l'unico innesco: `.github/workflows/release.yml` costruisce, verifica e pubblica la release allegando l'APK debug, usando come descrizione `docs/release-notes/vX.Y.Z.md`. Le note di rilascio si scrivono prima del tag, mai dopo.
