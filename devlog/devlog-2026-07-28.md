...
## 2026-07-28 - Preparazione della release 1.0.0 su GitHub

**Fatto:** l'app va in 1.0. Il canale di distribuzione non e il Play Store (nessun account sviluppatore aperto) ma una release su GitHub con l'APK di debug allegato, cioe lo stesso repository che fa da vetrina del progetto. Preparata tutta la parte che precede il tag: versione a `versionCode` 150 e `versionName` 1.0.0; note di rilascio complete in `docs/release-notes/v1.0.0.md` (cosa c'e nella 1.0, download e installazione, privacy, note tecniche, limitazioni note, riepilogo in inglese); nuovo workflow `.github/workflows/release.yml` che al push di un tag `v*` costruisce e verifica (`assembleDebug testDebugUnitTest lint detekt`), rinomina l'artefatto in `saldo-<versione>-debug.apk` e pubblica la release usando il file di note versionato; CI ristretta ai push su branch, cosi il tag non innesca due build identiche; README con badge di release, link al download e sezione di installazione. PLANNING riorganizzato: la Fase 10 diventa la release su GitHub ed e stata spostata in fondo al blocco delle fasi implementate (dopo le 10.x e la Fase 11, che si chiudono prima di lei); la roadmap v1.5 e stata chiusa, con le voci gia fatte raccolte nella nuova Fase 9.16 e quelle aperte confluite nella Roadmap v2.0; le fasi ancora da sviluppare sono 12, 13, 14, la Fase cloud (da valutare) e la nuova Fase 15, la release v2.0, con due strade distinte, GitHub e Play Store.

**Decisioni:** ADR 38. Si distribuisce la build di debug e non una build di release perche e esattamente quella provata su device e in CI a ogni push: attivare R8 e le sue regole (Room, kotlinx.serialization, Glance, Vico) all'ultimo passo aggiungerebbe l'unico rischio non coperto dai test proprio alla prima cosa che un estraneo installa. Il keystore di debug pubblico non certifica identita e non pretende di farlo: serve a garantire l'aggiornamento in place tra una versione e l'altra. Il tag come unico innesco evita la release costruita a mano da una macchina di sviluppo, che e il modo classico per pubblicare un artefatto che non corrisponde al codice taggato. I passi Play Store non sono stati cancellati ma spostati nella Fase 15 come strada B, con l'avvertenza che i due canali usano certificati diversi e quindi non si aggiornano a vicenda. Dalla 1.0.0 il `versionName` delle build di sviluppo prosegue come `1.0.<incremento>` (regola aggiornata in CLAUDE.md).

**Problemi:** nessuno. Resta aperto il baseline profile, rinviato alla Fase 15: richiede un modulo macrobenchmark e una generazione su device o emulatore, non disponibili in questo ambiente. L'unico effetto e un primo avvio leggermente piu lento, dichiarato tra le limitazioni note delle note di rilascio.

**Verificato:** `assembleDebug testDebugUnitTest lint detekt` in locale con il Gradle preinstallato. Le verifiche su device restano da fare prima del tag e sono elencate nella Fase 10: QA end-to-end, checklist widget ancora aperte delle Fasi 10.19-10.21, `MigrationsTest` strumentato, round-trip di backup su dati veri, installazione pulita.

**Prossimo:** eseguire la checklist di verifica su device, poi merge su `main`, tag annotato `v1.0.0` e push del tag; a release pubblicata, scaricare l'APK e installarlo su un device pulito.
