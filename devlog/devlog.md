...
**Decisioni:** nessuna migration e nessun bump di `SALDO_DATABASE_VERSION`: il tipo è persistito come stringa (`Converters` usa `valueOf`) e lo schema non cambia; `BackupValidation` valida già con `AccountType.valueOf`, quindi un file con `LOAN` passa e i file vecchi restano validi (coperto da test del codec). La scheda del prestito non compare senza una regola ricorrente verso il conto (senza rata non c'è nulla da stimare), con un'eccezione voluta: lo stato "estinto" si mostra anche senza regola, perché è l'informazione che chiude il ciclo di vita del conto. Un rimborso oltre il dovuto (saldo positivo) si presenta come estinto, mai come debito negativo: il residuo è azzerato a livello di use case e il saldo reale resta visibile nella riga del conto. Il campo importo dell'editor accettava già il segno meno (`allowNegative` è il default per il saldo iniziale, serve agli scoperti): per il prestito è bastata la validazione, senza toccare `MoneyInput`.

**Problemi:** nessuno di rilievo. L'unico scostamento dal piano è che il passaggio "abilita `allowNegative` per LOAN" previsto in PLANNING non è servito: era già il comportamento del campo.

**Verificato:** unit test JVM nuovi su preset di inclusione (preset off, override utente che vince, valore persistito mai sovrascritto in modifica), validazione del saldo iniziale (vuoto/zero/positivo rifiutati, negativo salvato, tipi ordinari e ramo carta di credito invariati), use case (residuo e frazione, arrotondamento per eccesso delle rate mancanti, nessuna regola, più regole sommate, valuta diversa e destinazione diversa escluse, estinto anche oltre il dovuto, archiviati e non-prestiti fuori) e round-trip del codec di backup con `LOAN`; i test degli obiettivi di risparmio girano invariati sull'estrazione condivisa. Verifica completa `assembleDebug testDebugUnitTest lint detekt` delegata alla CI GitHub (nessun SDK locale su questa macchina). Bump a versionCode 146, versionName 0.9.107.

**Prossimo:** Fase 12 (crediti e debiti verso persone); la Fase 10.20 ha ancora la verifica su device in sospeso.
...
**Decisioni:** tre meritano il perché. (1) Il refresh "senza logica" aveva due gambe: Exact fa passare ogni resize dal processo dell'app (broadcast, sessione Glance dentro WorkManager, ricomposizione), e una sessione Glance vive circa 45 secondi, quindi entro quella finestra tutto è istantaneo e fuori si paga lo scheduling di WorkManager, variabile per definizione. La storia di questo widget è Responsive -> Exact -> Responsive, e non è un pendolo: la prima Responsive aveva tre bucket disegnati a mano e le righe non crescevano; Exact ha dato la dimensione vera al prezzo del round-trip; ora i gradini di `layoutFor` (che è sempre stata una funzione a gradini) diventano gli 11 bucket di `WidgetBuckets`, così la crescita delle righe sopravvive e il resize torna a essere un affare del launcher. Il tetto di piattaforma è 16 size per RemoteViews, siamo a 11; il contenuto si compone una volta per bucket, quindi `loadShared` chiavato su (config, revisione) collassa 11 composizioni in una lettura, e può permettersi la cache che a `load` resta vietata perché la revisione è la versione dei dati. (2) Il tema: un widget vive nel processo del launcher, e il launcher ri-risolve da solo le risorse day/night quando il sistema cambia tema, ma solo se gli sono stati consegnati entrambi i rami. Lo sfondo era un `Color` risolto alla composizione e i glifi bitmap con il colore cotto dentro: il widget restava nel tema vecchio finché qualcosa non lo ridisegnava. Ora lo sfondo e ogni tinta sono `ColorProvider` a due rami e i glifi sono maschere bianche tintate con `ColorFilter.tint`, quindi il flip è gratis e senza svegliare nessuno; un appearance forzato passa lo stesso schema su entrambi i rami, come prima. In più le preferenze tema sono entrate nei `refreshSignals`: cambiare tema dall'app ridisegna subito. (3) Lo slider di opacità torna dopo la rimozione della v0.9.94, ma non è lo stesso slider: quello vecchio lasciava all'inchiostro il tema dell'app, questo sotto il 50% di opacità prende il lato dal hint `HINT_SUPPORTS_DARK_TEXT` del wallpaper (lo stesso che decide il colore dell'orologio, nessun permesso) e addensa le velature delle tile dal 16% verso il 38% mentre lo sfondo svanisce, perché il hint è globale e non sa dove sta il widget: il lato giusto lo sceglie lui, il contrasto locale lo garantiscono le velature. Il valore legacy TRANSPARENT si legge come Sistema a opacità zero, nessuna migrazione. Sull'icona dell'app ho deciso per il bottone tonale quieto invece del marchio nudo: su sfondo trasparente un'icona senza fondo galleggia sul wallpaper e non legge come controllo.

**Problemi:** `providePreview` e `setWidgetPreviews` sono API di Glance 1.2 verificate sui riferimenti ufficiali ma mai provate in questo progetto: se il preview generato desse noie, il fallback è il `previewLayout` XML che resta al suo posto. Il worker di mezzanotte si ricalendarizza da dentro il proprio `doWork` con `APPEND_OR_REPLACE`, perché `REPLACE` cancellerebbe il worker stesso mentre gira.

**Verificato:** verifica statica accurata in locale (nessun SDK Android su questa macchina); build, unit test, lint e detekt delegati alla CI del PR. Test nuovi: bucket->layout e fontScale in `WidgetLayoutTest`, legacy TRANSPARENT e clamp opacità in `QuickAddWidgetPrefsTest`, segnale tema in `WidgetRefreshWatcherTest`, entrate di oggi e `loadShared` in `QuickAddWidgetDataLoaderTest`; strumentati riscritti su coppia light/dark, opacità e maschera bianca. versionCode 139 -> 140, versionName 0.9.100 -> 0.9.101.

**Prossimo:** prova su device: resize tra i bucket (deve essere istantaneo), flip del tema di sistema a widget fermo, slider a 0% su wallpaper chiaro e scuro, rollover del totale dopo mezzanotte, preview generata nel picker.
