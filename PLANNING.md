...
- [x] Tipografia: famiglia Inter (variable font OFL embeddato in `res/font`); headline/title a peso SemiBold in `SaldoTypography`; titoli delle righe impostazioni a Medium per staccarli dalla descrizione; sugli importi `tabularNumbers()` con `tnum` (figure tabulari) + `zero` (zero barrato)
