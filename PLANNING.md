...
- [x] Tipografia: famiglia Inter (variable font OFL embeddato in `res/font`), asse `opsz` legato alla dimensione di ogni stile; headline/title a peso SemiBold in `SaldoTypography`; sugli importi `tabularNumbers()` con `tnum` (figure tabulari) + `zero` (zero barrato)
