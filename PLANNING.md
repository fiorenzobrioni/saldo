...
- [ ] **Da verificare su device**: nel dialog della data compaiono i chip rapidi "Oggi"/"Ieri"? Lo screenshot dell'utente suggerisce di no, e vivono nello stesso slot `content` dove la riga dell'ora non si vedeva (in material3 1.4.0 quello slot finisce in un `Box(Modifier.weight(1f, false))`). Se confermato e un difetto che risale alla Fase 10.12, da correggere con una conferma visiva in mano
